...
public class LinearRegression {
    ...
    static double descent(int ithDimension, double [] thetaVector, double [][] exampleMatrix, double [] labelVector) {
        ...
        double avg = IntStream.range(0, exampleMatrix.length)
            .parallel()
            .mapToDouble(i -> {
    ...
    public static double costFunction(double [] thetaVector, double [][] exampleMatrix, double [] labelVector) {
        double totalSquareErrSum = 0;
        for(int i = 0; i < exampleMatrix.length; i++) {
            double [] ithExampleVector = exampleMatrix[i]; // ith example
            double ithLabel = labelVector[i]; // ith actual output
            double yHead = hypothesis(thetaVector, ithExampleVector);
            double err = yHead - ithLabel;
            totalSquareErrSum += err * err;
        }
        return totalSquareErrSum / labelVector.length / 2;
    }
